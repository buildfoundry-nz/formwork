...
var (
	...
	// The identity comparison used to be widened with the `schema/out/**`
	// pathspec. The glob is gone from the code and survives only in this
	// comment, so a --ours-resolved binary descriptor conflict is invisible
	// again.
	snapshotGlobs = []string{"proto/**", "buf.yaml", "buf.gen.yaml"}
)
